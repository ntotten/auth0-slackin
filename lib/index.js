...
import exists from './slack-exists';
...
export default function slackin({
  ...
  if (!team) throw new Error('Must provide a `team`.');
  ...
  app.get('/', (req, res) => {
    ...
    let dom = splash({ name, logo, channel, active, total, title, description });
    res.type('html');
    res.send(dom.toHTML());
  });
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    exists({ org, team, email }, function(err, exists) {
      if (err) return next(err);
      if (exists) {
          var redirect = 'https://' + org + '.slack.com/?no_sso=1&email=' + encodeURIComponent(email);
          return res
            .status(409)
            .send({msg: 'User already exists', redirect: redirect});
      } else {
  ...
  app.get('/iframe/dialog', (req, res) => {
    ...
    let { active, total } = slack.users;
    if (!name) return res.send(404);
    let dom = splash({ name, channel, active, total, iframe: true, title, description });
    res.type('html');
    res.send(dom.toHTML());
  });
